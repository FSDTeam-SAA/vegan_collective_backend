...
exports.getReviewsByProduct = async (req, res) => {
  try {
    ...
    const reviews = await MerchantProductsReview.find({ productID }).populate("userID", "name");
...
exports.getAverageRating = async (req, res) => {
  try {
    ...
    res.status(200).json({ averageRating: avgRating.toFixed(2) });
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error", error });
  }
};
...
async function updateProductAverageRating(productID) {
  ...
  await MerchantProducts.findByIdAndUpdate(productID, { avgRating: avgRating.toFixed(2) });
}
